...
`;
    ...
    // 发送带有标记的 Padding，强制 Proxy 层开启流式传输，标记将被前端过滤
    res.write("<!-- STREAM_START -->" + " ".repeat(1024) + "\n");
        ...
        if (match) {
          ...
          try {
            ...
            if (content) {
              // 仅将纯文本推流给前端
              res.write(content);
              // 如果环境支持 flush (如某些 proxy 层)，尝试触发
              if (res.flush) res.flush();
            }
          } catch (e) {
            // 解析失败通常是因为 chunk 不完整，记录但继续
